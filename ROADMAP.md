...
- [ ] 通知メッセージの多言語対応: `NOTIFY_LANG` 環境変数で ja / en を切り替え可能にする
  - 対象: Slack通知メッセージのみ（ログ出力は対象外）
  - 初期対応: ja（日本語）/ en（英語）の2言語
  - 実装: `common/messages/` 配下に言語別テンプレート辞書を配置
  - 各サービスの通知組み立て部分をテンプレート参照に置き換え
  - デフォルト `ja` で既存ユーザーに影響なし
  - 追加言語はPR歓迎（テンプレートファイル追加のみで対応可能な設計にする）
