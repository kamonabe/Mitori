...
def fetch_unprocessed(conn):
    with conn.cursor() as cur:
        cur.execute(
            "SELECT id, raw_json FROM mitre_raw_staging WHERE source=%s AND processed_at IS NULL", (COLLECTION_KEY,)
        )
        return cur.fetchall()
...
def upsert_tactic(conn, n):
    """
    tacticをupsertし、変更があった場合は変更イベントdictを返す。
    変更なしの場合は None を返す。
    """
    content_hash = compute_hash(n)
    with conn.cursor() as cur:
        cur.execute("SELECT id, content_hash, is_deprecated FROM mitre_tactics WHERE stix_id=%s", (n["stix_id"],))
        row = cur.fetchone()
...
def upsert_technique(conn, n):
    """
    techniqueをupsertし、変更があった場合は変更イベントdictを返す。
    変更なしの場合は None を返す。
    """
    content_hash = compute_hash({k: v for k, v in n.items() if k != "tactic_keys"})
    with conn.cursor() as cur:
        cur.execute(
            "SELECT id, content_hash, is_deprecated, is_revoked FROM mitre_techniques WHERE stix_id=%s", (n["stix_id"],)
        )
        row = cur.fetchone()
        ...
        if row is None:
            ...
            # action を判定: deprecated > revoked > updated の優先度
            if not row["is_deprecated"] and n["is_deprecated"]:
                action = "deprecated"
...
def sync_tactic_map(conn, technique_id, tactic_keys):
    with conn.cursor() as cur:
        ...
        cur.execute(f"SELECT id, tactic_key FROM mitre_tactics WHERE tactic_key IN ({placeholders})", tactic_keys)
...
def build_category_block(label: str, events: list) -> str:
    """
    1カテゴリ分のメッセージブロックを生成する。
    NOTIFY_MAX_ITEMS 件まで個別列挙し、超えた場合は「上記も含め、N件の変更を検知しました」を付加。
    """
    total = len(events)
...
def notify_slack(events: list):
    """変更イベントをカテゴリごとに整理してSlackへ通知する。"""
...
def mark_processed(conn, ids):
    if not ids:
        return
    with conn.cursor() as cur:
        cur.executemany("UPDATE mitre_raw_staging SET processed_at=NOW() WHERE id=%s", [(i,) for i in ids])
    conn.commit()


def cleanup_old_processed(conn):
    """処理済みレコードのうち CLEANUP_RETENTION_DAYS 日以上経過したものを削除する。"""
    with conn.cursor() as cur:
        cur.execute(
            "DELETE FROM mitre_raw_staging WHERE processed_at IS NOT NULL AND processed_at < NOW() - INTERVAL %s DAY",
            (CLEANUP_RETENTION_DAYS,),
        )
        deleted = cur.rowcount
    conn.commit()
    if deleted > 0:
        print(f"Cleanup: {deleted}件の処理済みレコードを削除しました（{CLEANUP_RETENTION_DAYS}日超過分）")


def update_schedule(conn, had_changes: bool):
    with conn.cursor() as cur:
        cur.execute("SELECT backoff_minutes FROM mitre_taxii_cursor WHERE collection_id=%s", (COLLECTION_KEY,))
        row = cur.fetchone()
...
def main():
    ...
    # Pass 2: techniqueを処理(この時点でtacticは全部揃っている)
    for row_id, obj_type, obj in parsed_rows:
