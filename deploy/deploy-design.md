...
```
実行元ホスト ── SSH ──▶ k3s 制御ノード
   pyinfra                 kubectl / helm（kubeconfig 有効）
      │
      ├─ inventory.py      接続先ホストの定義（IP / ユーザー等）
      ├─ deploy_cluster.py デプロイ操作の定義（operations）
      └─ .env              Secret 実値（gitignore 済み）
```

pyinfra は「実行元で操作を組み立て、SSH 越しに対象ホストで `kubectl` / `helm` を実行する」構成。対象ホスト上で直接実行する場合は `@local` を使う。

前提となるのは次の2点のみで、実行元の OS や接続先のネットワークセグメントは問わない。

- **実行元**: Python と pyinfra が動くこと（OS は不問）
- **接続先**: SSH で到達でき、その上で `kubectl` / `helm` が使えること

接続先の IP アドレスやユーザー名などの環境固有の値は設計上の制約ではなく、`inventory.py` に設定する。別環境へ適用する場合は `inventory.py` を書き換えるだけでよい。

### 3.1 実行権限（SSH 先ユーザーに必要な権限）

`inventory.py` の `ssh_user` に指定するユーザーが必要とする権限を、OS レベルと Kubernetes レベルに分けて示す。

#### OS レベル: sudo は不要

本スクリプトは対象ホスト上で `kubectl` / `helm` を実行するだけで、OS 側の操作（パッケージインストール・systemd 操作・ファイアウォール設定など）は行わない。したがって **sudo / root 権限は不要**。一般ユーザーで実行できる。

> OS レベルの操作を伴うホスト構築（k3s / helm のインストール等）は本スクリプトのスコープ外（第 2.2 章）。そちらは `SETUP.md` に従い別途 sudo で行う。

#### kubeconfig の読み取り権限

SSH 先ユーザーが有効な kubeconfig を参照できること。
...
2. pyinfra inventory.py deploy_cluster.py
...
DB テーブルは永続化されているため初期化は不要。`pyinfra inventory.py deploy_cluster.py` を通しで実行するだけでよい。
...
```bash
pyinfra inventory.py deploy_cluster.py --dry
```
